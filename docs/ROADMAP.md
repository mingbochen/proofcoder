...
- 规范版本：v3.3
- 当前阶段：阶段 I（多轮会话）
- 下一步：小项 I.2，会话存储、携带摘要的组装与冻结，以及证据不跨运行继承的保证
- 最近完成：小项 I.1，阶段 ADR 与规范补全（[ADR-0007](adr/0007-cross-run-sessions.md)、规范 §9.5、§10.6 与 §13.6）
- 阻塞项：无
...
| I.2 | 会话存储与校验、携带摘要的确定性组装与裁剪、证据不跨运行继承的保证，含离线测试 | 未开始 | — |
| I.3 | 命令行与浏览器的会话入口，以及浏览器在服务重启后的会话恢复 | 未开始 | — |
