...
- 下一步：小项 H.2，三值判定、策略加载与审批协议
- 最近完成：小项 H.1，阶段 ADR 与规范补全（[ADR-0006](adr/0006-command-policy-and-approval.md)、规范 §10.4 与 §13.5）
- 阻塞项：无
...
| H.2 | 三值判定、项目策略加载与冻结、同步主循环中的审批协议，含离线测试 | 未开始 | — |
| H.3 | 命令行与浏览器的审批入口，含摘要绑定 | 未开始 | — |
