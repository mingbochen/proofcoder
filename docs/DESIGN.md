...
| [configuration](../src/proofcoder/config.py), [provider factory](../src/proofcoder/llm/factory.py), [DeepSeek client](../src/proofcoder/llm/deepseek.py), and [local-model client](../src/proofcoder/llm/ollama.py) | provider selection and per-provider environment groups, synchronous transport, response normalization, API error classification | local retries in an SDK, tool execution, history ownership, or any decision the loop owns | AgentLoop and the providers |
...
A second provider exists to test the seam rather than to add a vendor. The local-model adapter speaks Ollama's native chat API through the standard library, which forces three real conversions: tool-call arguments arrive as a JSON object where the rest of ProofCoder carries the model's untouched JSON string, tool calls carry no identifier so one is synthesized for the loop's call-and-result pairing, and tool results are addressed by name rather than by id. Provider failures are translated into the same error categories the retry policy already reads, so that policy names no provider. What the loop sees is identical either way, and a test drives the same trajectory through both adapters and compares the event sequences.

Streaming is applied by wrapping rather than by a branch anywhere inside the loop. A streaming-capable client is presented through the ordinary `complete` seam by a small adapter that consumes the stream, feeds an assembler and returns the same response object the non-streaming path returns. So `AgentLoop` has no streaming code and no knowledge that a stream occurred; the recorded trace is identical either way, and the committed golden trace needs no regeneration. The assembler merges tool-call fragments by index, fixes each identifier and function name the first time it sees them, and parses the accumulated arguments exactly once when they are complete — a partially read argument string can only be guessed at, and a guessed argument could never afterwards be distinguished from one the model really asked for. Every failure mode yields no tool call at all rather than part of a batch. Only visible text reaches the incremental callback.
