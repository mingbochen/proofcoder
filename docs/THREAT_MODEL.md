...
| TM-29 | Unintended rollback | A rollback is triggered without the operator understanding its scope and discards work the run did not produce, such as a concurrent edit made while the run was in progress. | Rollback is never automatic and no tool exposes it to the model; the full plan is printed before anything is written, with each path labelled by the source the trace can prove; confirmation is required, `--yes` is an explicit unattended choice, and a non-terminal standard input refuses rather than assuming an answer; the operation records its own trace naming the run it undid. | [rollback](../src/proofcoder/rollback.py), [rollback CLI tests](../tests/unit/test_cli_rollback.py) | Mitigated | A confirmed rollback still reverts any in-scope change made during the run, whoever made it. The plan is the place to notice that, and an operator who passes `--yes` has chosen not to look. |
