...
| Local web interface surface | Reachable only from this machine's own browser page; the session token must stay out of URLs, logs, and terminal output; a page from any other origin must not be able to start, read, or stop a run. |
...
flowchart TD
    ...
    B["Local browser page (token, Host, and Origin checked)"] --> CLI
...
| TM-23 | Allowed Python script | The model runs an allowlisted workspace `.py` file that performs arbitrary OS actions. | Script path must be a regular workspace file; argv, cwd, environment, timeout, output, and process handling remain constrained; limitation is disclosed in the tool schema; writes it makes inside the captured scope are recoverable from the run checkpoint. | [command policy](../src/proofcoder/safety/commands.py), [policy tests](../tests/unit/test_command_policy.py), [checkpoint tests](../tests/unit/test_checkpoint.py) | Accepted boundary | The script has current-user authority and can access resources outside application path policy; the checkpoint cannot recover anything it wrote outside the workspace. |
| TM-24 | Local web surface | A page on another origin, or another process on the host, drives the agent through the serve endpoint to read, write, execute, or roll back in the workspace. | Loopback bind by default with a printed warning otherwise; a per-process `secrets.token_urlsafe` session token required on every `/api` request and written only into the page served from this origin; constant-time comparison; exact `Host` and `Origin` matching against the bound address; bounded request bodies; strict content-security policy with no external asset. | [web server](../src/proofcoder/web/server.py), [server tests](../tests/unit/test_web_server.py) | Partially mitigated | Any local process that can read this process's memory, terminal, or the served page can obtain the token, and a non-loopback bind exposes the surface to that network. The token lives only in process memory and the page, so it is lost on restart rather than revoked. |
| TM-25 | Browser-started run lifecycle | A browser starts unbounded concurrent runs, floods memory with buffered events, or cannot stop a run it started. | One run per workspace and a concurrent-run cap; task size bound; the same bounded step/time/context/failure limits the CLI enforces, validated server-side; a fixed-size event buffer that counts drops instead of growing; cooperative cancellation polled between model calls and between tool calls, reported as the existing interrupted termination. | [sessions](../src/proofcoder/web/sessions.py), [session tests](../tests/unit/test_web_sessions.py), [cancellation tests](../tests/unit/test_agent_cancellation.py) | Mitigated | Cancellation is cooperative, so an in-flight provider request or local command completes before the run stops. |
| TM-29 | Unintended rollback | A rollback is triggered without the operator understanding its scope and discards work the run did not produce, such as a concurrent edit made while the run was in progress. | Rollback is never automatic and no tool exposes it to the model; the full plan is printed before anything is written, with each path labelled by the source the trace can prove; confirmation is required, `--yes` is an explicit unattended choice, and a non-terminal standard input refuses rather than assuming an answer; in the browser the confirmation carries the digest of the displayed plan and the server refuses any request whose digest no longer matches the plan that would run, returning the new plan for review; a workspace with a run in progress is refused; the operation records its own trace naming the run it undid. | [rollback](../src/proofcoder/rollback.py), [rollback CLI tests](../tests/unit/test_cli_rollback.py), [browser rollback tests](../tests/unit/test_web_rollback.py) | Mitigated | A confirmed rollback still reverts any in-scope change made during the run, whoever made it. The plan is the place to notice that, and an operator who passes `--yes` has chosen not to look. |
| TM-30 | Destructive file tools | The model deletes, moves onto, or overwrites a path and destroys content in one call, including content the checkpoint cannot restore. | Deletion takes one file or one empty directory and never recurses, so a subtree cannot be lost in one step; moving never overwrites its destination; overwriting is refused unless the call explicitly asks for it; all three refuse outright when the run has no checkpoint, decided by the run rather than by any argument the model can supply; every path passes the same workspace, symlink, and sensitive-path checks as the other file tools. | [path tools](../src/proofcoder/tools/paths.py), [path tool tests](../tests/unit/test_path_tools.py), [rollback coverage test](../tests/unit/test_checkpoint_runtime.py) | Mitigated | Within the captured scope the damage is reversible, but rollback is a separate user action: until someone runs it, the workspace stays as the run left it. Content outside that scope -- credential paths, oversized files, ignored directories -- is destroyed for good if a workspace script reaches it, since these tools refuse those paths but the command tool is not bound by them. |
| TM-27 | Checkpoint content exposure | The pre-run baseline copies workspace files into `.proofcoder/checkpoints`, turning the runtime directory into a second copy of whatever the workspace holds, including credentials. | Sensitive paths contribute neither content nor a content digest and are recorded by metadata alone; blobs and manifests are written under `0o700` run directories inside the ignored runtime tree; retention is bounded by run count and total bytes and pruned before each capture. | [checkpoint](../src/proofcoder/checkpoint.py), [checkpoint tests](../tests/unit/test_checkpoint.py) | Partially mitigated | A workspace file that holds a secret but does not match the sensitive-path rules is copied like any other file, and directory modes do not protect a backup, sync, or archive of the workspace. |
| TM-28 | Incomplete or unsafe rollback | A rollback restores part of a workspace, follows a path out of it, or writes back content that no longer matches what was captured, leaving a state nobody can reason about. | Planned paths are revalidated and contained at apply time; stored content is verified against its digest before any write; writes reuse the atomic staging path; each action is independent and every failure is reported by path and code; repeating a rollback is a no-op. | [checkpoint](../src/proofcoder/checkpoint.py), [checkpoint tests](../tests/unit/test_checkpoint.py) | Mitigated | A rollback restores only the captured scope: credential paths, files over the size limit, ignored directories, symbolic links, and anything outside the workspace stay as the run left them, and `--no-checkpoint` removes the protection entirely. |
| TM-26 | Platform/provider compromise | OS, interpreter, current account, provider, trusted executable, package source, or runner is compromised. | No complete local control; minimize privileges, review provenance, and rotate credentials after exposure. | [COMPLIANCE limitations](COMPLIANCE.md#11-security-boundaries-and-limitations) | Out of scope | Such compromise can bypass policy, alter evidence, or disclose credentials. |
...
A run checkpoint is a recovery aid within its captured scope, not a backup and not a reason to relax any of the above. It stores no credential content, so a workspace script that destroys `.env` or a private key destroys it for good; ProofCoder reports the change and cannot undo it. Keep independent backups of anything irreplaceable, and treat the checkpoint directory as carrying the same sensitivity as the workspace it copies.
...
- A run captures its checkpoint before the first model call or ends as `checkpoint_error`; sensitive paths contribute no content or digest to it, and no rollback writes, deletes, or recreates one.
- A tool that destroys content in one call is available only when the run holds a checkpoint, and deletion never recurses.
- Rollback stays a user action against a finished run: no tool exposes it to the model, and no uncovered or failed path is omitted from its reported result.
- A rollback prints its complete plan and obtains confirmation before its first write, and records the operation under its own run identifier rather than appending to the trace it undoes.
- An interface that cannot ask at the moment of the decision binds the confirmation to the plan it displayed, and the server rechecks that binding before writing rather than trusting the claim.
