...
| v3.4 | 2026-09-15 | 新增 §4.7 与 §11.5；§4.2、§4.6、§13.1 与 §15.2 补充提供方与流式条目；细化阶段 J 的实现清单与退出条件 | ADR-0008 |
...
- 默认使用非流式响应，避免流式工具参数拼接增加协议复杂度。阶段 J 起可以按运行显式开启流式，约束见 §4.7。

普通 `openai` 客户端仅负责请求序列化、HTTP 通信和响应对象构造。创建客户端时显式设置 `max_retries=0`，API 重试由项目代码统一处理。

以上是默认提供方的配置。阶段 J 起可以选择另一个提供方，选择方式与约束见 §4.7；不做选择时行为与阶段 J 之前一致。

### 4.7 模型提供方与流式响应

阶段 J 起，模型提供方可以替换，响应可以流式获取。约束取舍见 [ADR-0008](adr/0008-provider-abstraction-and-streaming.md)。

#### 4.7.1 提供方的选择与边界

- 提供方由 `PROOFCODER_PROVIDER` 选择。不设置时使用默认提供方，行为与阶段 J 之前逐字节一致。
- 每个提供方使用自己的一组环境变量，不与其他提供方共用变量名。共用会让「哪个变量对哪个提供方生效」变成必须读代码才能确定的事。
- 提供方适配层只承担两件事：**通信**，以及**协议对象转换**。进来的是本仓库自己的消息与工具 schema，出去的是本仓库自己的响应对象；提供方特有的对象不得越过适配层。
- 适配层不得重试，不得决定终止，不得判定完成，不得改写历史。这些属于 §2.2 必须自行实现的核心逻辑。
- 提供方的失败必须翻译成既有的错误类别，重试策略据此工作，因此重试策略中不得出现任何提供方名称。
- 至少有一个提供方不依赖云端凭据，使真实模型路径在没有 key 的环境中也能被验证。

#### 4.7.2 流式响应

- 流由适配层消费，拼装并校验成与非流式**相同**的响应对象之后，才交给 AgentLoop。AgentLoop 不知道一次响应是否来自流。
- 流式只影响呈现，不影响判定。上下文计量、协议修复、工具批次校验只有一条路径。
- 适配层可以接受一个增量回调，把已收到的**可见文本**交给调用方。tool call 片段不进入该回调：半个 tool call 不是给人看的内容，显示它会让人以为某个操作已经开始。
- 流式按运行显式开启，非流式始终可用且是默认。评测一律走非流式路径，不提供开关。

#### 4.7.3 流式 tool call 的拼装

- 片段按提供方给出的 index 归并。id 与函数名在第一次出现时确定，之后出现不同的值是协议错误，不是覆盖。
- 参数按字符串拼接，拼装完成后整体解析一次。不做增量解析，不对不完整的参数作任何推断。
- 片段数量与参数总长度有上限。
- 任何一种拼装失败都不得产生部分执行：要么整批 tool call 合法并交给 AgentLoop，要么这次模型调用整体失败。失败的分类见 §11.5。
...
- `PROOFCODER_PROVIDER`：默认提供方标识，取值见 §4.7。不设置时使用默认提供方。
...
- `run` 执行一次 agent 任务。阶段 H 起接受审批模式参数，以及显式指定项目命令策略文件的参数；两者的约束见 §10.4.2 与 §10.4.3。阶段 I 起接受显式指定会话的参数，约束见 §9.5 与 §10.6；不指定时不读也不写任何会话数据。阶段 J 起接受开启流式响应的参数，约束见 §4.7；不指定时走非流式路径。
- `trace` 回放脱敏运行轨迹。
...
阶段 J 起，流式拼装的每一种失败都必须落进 §11.1 的既有层级，不新增层级：

| 失败 | 层级 | 处理 |
|---|---|---|
| 连接中断、读取超时 | 瞬时 API 错误 | 按既有策略有限重试，重试仍走流式 |
| 片段数量或参数长度超限 | 永久 API 错误 | 不重试，向用户报告可行动原因 |
| 拼装完成后参数不是合法 JSON | 参数与协议错误 | 本地生成结构化结果，按既有协议修复路径处理 |
| 同一 index 的 id 或函数名中途改变 | 永久 API 错误 | 不重试；这是提供方违反协议，重试不会改变 |
| 流在拼装完成前结束 | 瞬时 API 错误 | 按既有策略有限重试 |

- 以上任何一种都不得产生工具执行。
- 流式失败不得自动改用非流式重发：那会把一次失败变成两次请求，而轨迹中看不出发生过什么。
...
- `TASK`：原始任务。
- `MODEL`：模型对用户可见的简短文本。阶段 J 起，流式运行可以在事件提交之前增量呈现这段文本；增量呈现不产生额外事件，也不改变事件内容。
- `TOOL`：工具名与脱敏参数。
...
8. 同一段响应经流式与非流式两条路径，拼装出的响应对象逐字段相等。
9. §11.5 列出的每一种流式拼装失败，各自返回确定的结构化错误且不产生任何工具执行。
10. 同一段脚本化响应序列经两个不同提供方的适配层进入循环，事件序列与终止结果相同。
...
约束见 §4.7 与 §11.5，方案取舍见 [ADR-0008](adr/0008-provider-abstraction-and-streaming.md)。

实现：

- 提供方选择与配置分组，默认提供方的行为不变。
- 第二个提供方适配层：本地模型，不依赖云端凭据，也不新增运行时依赖。
- 流式响应的消费、拼装与校验，拼装结果与非流式路径相同。
- 流式可见文本的增量回调，以及命令行与浏览器界面的增量渲染。
- 流式拼装失败按 §11.5 分类。
- 按提供方分叉的 `doctor` 检查。

退出条件：

- 提供方适配层只负责通信和协议对象转换，AgentLoop 的行为与提供方无关：同一段脚本化响应序列经两个适配层产生相同的事件序列与终止结果。
- 流式拼接中的中断、截断和非法 JSON 都转为结构化错误，并有协议回归测试；任何一种都不产生工具执行。
- 同一段响应经流式与非流式拼装出的响应对象逐字段相等。
- 非流式路径继续可用，作为默认而不是回退；评测一律走非流式路径。
- 重试策略中不出现任何提供方名称。
- 不选择提供方、不开启流式时，金标轨迹不需要重新生成。
- 至少一个提供方不依赖云端凭据，使真实模型路径在没有 key 的环境中也可验证。
