...
- 每次运行开始前为工作区建立检查点。
- 回滚一次运行造成的文件变更，包括内置工具和被允许的工作区进程造成的变更。
- 命令行与浏览器界面的回滚入口。
- 检查点的保留、清理和存储上限。
...
- 检查点不得把凭据内容写入运行产物，敏感文件的处理方式在阶段 ADR 中确定。
...
- 项目级命令策略：在默认拒绝的基础上声明额外允许的命令。
- “需要确认”的命令类别，以及命令行和浏览器中的审批流程。
- 同步主循环等待审批时的超时、拒绝和中断语义。
...
- 审批请求、审批决定和执行结果都进入轨迹。
- 模型不能通过文件工具或命令工具放宽策略。
- 新增至少一个非 Python 项目的评测 fixture。

### 阶段 I：多轮会话

实现：

- 同一工作区内跨运行的会话：规定携带历史或程序生成摘要的规则。
- 会话的创建、继续、结束与清理，以及命令行和浏览器入口。

退出条件：

- 后续运行能利用前序运行的结论，但验证证据不跨运行继承：上一次运行的验证不能让本次运行成为 `completed_verified`。
- 携带到新运行的内容受上下文预算约束，并经过与轨迹相同的脱敏。
- 服务重启后，会话能从持久化数据恢复。
